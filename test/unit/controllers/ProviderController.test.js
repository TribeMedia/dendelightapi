...
describe('ProviderController', function() {
	...
	before(function(done) {
		...
		Admin.create({email: "admin_test_provider@gmail.com", password: "14491992"}, function(err, admin) {
	...
	describe('#fetch()', function () {
		...
		it('should reponse with a json list of providers', function(done) {
			request(sails.hooks.http.app)
				...
				.send({services: ['mowing', 'leaf_removal'], address: '16 Keats Avenue, Kingsbury', bookTime: 1469977804187, estimatedDuration: 1440000})
				.expect(200)
				.end(done);			
		});
		...
		it('should reponse with badRequest', function(done) {
			request(sails.hooks.http.app)
				.post('/api/v1/providers')
				.send({address: '16 Keats Avenue, Kingsbury', bookTime: 1469977804187, estimatedDuration: 1440000})
				.expect(400)
				.end(done);			
		});
	});
	...
	describe('#create()', function() {
		it('should have  json response', function(done) {
			request(sails.hooks.http.app)
				...
				.send({email: 'provider_create_test1@gmail.com', password: '14491992', firstName: 'Tom', lastName: 'Henry', abn: 'ud73gs37e8u', address: '16 Keats Avenue, Kingsbury'})
				.expect(201)
				.expect(hasProviderKey)
				.end(done);
